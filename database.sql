...
CREATE TABLE IF NOT EXISTS jelszo_reset (
  id              BIGINT UNSIGNED AUTO_INCREMENT PRIMARY KEY,
  felhasznalo_id  BIGINT UNSIGNED NOT NULL,
...
CREATE TABLE IF NOT EXISTS kuponok (
  ...
  min_osszeg          DECIMAL(10,2) DEFAULT 0,
  felhasznalasi_limit INT UNSIGNED DEFAULT NULL,
  felhasznalva        INT UNSIGNED DEFAULT 0,
  felhasznalo_id      BIGINT UNSIGNED DEFAULT NULL,
  ervenyes_kezdet     DATETIME DEFAULT NULL,
...
CREATE TABLE IF NOT EXISTS kupon_hasznalatok (
  id              BIGINT UNSIGNED AUTO_INCREMENT PRIMARY KEY,
  kupon_id        BIGINT UNSIGNED NOT NULL,
  felhasznalo_id  BIGINT UNSIGNED NOT NULL,
  rendeles_id     BIGINT UNSIGNED DEFAULT NULL,
  hasznalva       TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
  FOREIGN KEY (kupon_id) REFERENCES kuponok(id) ON DELETE CASCADE,
  FOREIGN KEY (felhasznalo_id) REFERENCES felhasznalok(id) ON DELETE CASCADE
);

CREATE TABLE IF NOT EXISTS fal_bejegyzesek (
  id              BIGINT UNSIGNED AUTO_INCREMENT PRIMARY KEY,
  felhasznalo_id  BIGINT UNSIGNED NOT NULL,
...
CREATE TABLE IF NOT EXISTS tiltott_szavak (
  id      BIGINT UNSIGNED AUTO_INCREMENT PRIMARY KEY,
  szo     VARCHAR(100) NOT NULL UNIQUE
);

INSERT INTO kuponok (kod, tipus, ertek, min_osszeg, felhasznalasi_limit, aktiv) VALUES
('HUSEG05', 'szazalek', 0.50, 0, NULL, 1),
('UJVASARLO10', 'szazalek', 10.00, 5000, 1, 1),
('NYAR2026', 'fix', 2000.00, 10000, 100, 1);

UPDATE felhasznalok SET admin = 1 WHERE id = 1;
